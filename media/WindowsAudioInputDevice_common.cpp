...
::WindowsAudioInputDevice_common(UsageEnvironment& env, int inputPortNumber,
  ...
  qDebug()<<"WindowsAudioInputDevice_common::WindowsAudioInputDevice_common";
}
...
void WindowsAudioInputDevice_common::doGetNextFrame() {
  qDebug()<<"WindowsAudioInputDevice_common::doGetNextFrame()";
  if (!fHaveStarted) {
...
void WindowsAudioInputDevice_common::onceAudioIsReady() {
  ...
  qDebug()<<"WindowsAudioInputDevice_common readFromBuffers fFrameSize="<< fFrameSize;
  if (fFrameSize == 0) {
...
#ifdef _MSC_VER
    unsigned thisRead = min((unsigned)(readHead->dwBytesRecorded - bytesUsedAtReadHead), (unsigned)(numBytesWanted - numBytesRead));
#else
    //unsigned thisRead = std::min(readHead->dwBytesRecorded - bytesUsedAtReadHead, numBytesWanted - numBytesRead);
    unsigned thisRead = std::min((unsigned)(readHead->dwBytesRecorded - bytesUsedAtReadHead), (unsigned)(numBytesWanted - numBytesRead));
#endif
