  ...
  do {
	...
	// THIS CODE WORKS ONLY FOR TCP: #####
        ...
        if (numExtraBytesNeeded > remainingBufferSize) {
          char tmpBuf[300];
          snprintf(tmpBuf, sizeof(tmpBuf),"Read buffer size (%d) is too small for \"Content-Length:\" %d (need a buffer size of >= %d bytes\n",
...
SIPClient::createAuthenticatorString(Authenticator const* authenticator,
  ...
  if (authenticator != NULL && authenticator->realm() != NULL
    ...
    //sprintf(authenticatorStr, authFmt,authenticator->username(), authenticator->realm(),
	  //  authenticator->nonce(), response, url);
	  snprintf(authenticatorStr,sizeof(authenticatorStr), authFmt,authenticator->username(), authenticator->realm(),
	    authenticator->nonce(), response, url);
