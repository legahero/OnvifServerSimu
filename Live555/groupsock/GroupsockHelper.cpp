...
int setupDatagramSocket(UsageEnvironment& env, Port port) {
  ...
  if (setsockopt(newSocket, SOL_SOCKET, SO_REUSEADDR,
    ...
    qDebug()<<"setupDatagramSocket setsockopt error,closeSocket :"<<newSocket;
    return -1;
  }
  ...
  if (setsockopt(newSocket, IPPROTO_IP, IP_MULTICAST_LOOP,
    ...
    closeSocket(newSocket);
    qDebug()<<"setupDatagramSocket setsockopt IP_MULTICAST_LOOP error,closeSocket :"<<newSocket;
    return -1;
  }
#endif
#endif
  ...
  if (port.num() != 0 || ReceivingInterfaceAddr != INADDR_ANY) {
    ...
    if (::bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
      ...
      closeSocket(newSocket);
      qDebug()<<"setupDatagramSocket bind error,closeSocket :"<<newSocket;
      return -1;
    }
#if defined(__WIN32__) || defined(_WIN32)
#else
  ...
  if (SendingInterfaceAddr != INADDR_ANY) {
    ...
    if (setsockopt(newSocket, IPPROTO_IP, IP_MULTICAST_IF,
      ...
      closeSocket(newSocket);
      qDebug()<<"setsockopt IP_MULTICAST_IF error,closeSocket :"<<newSocket;
      return -1;
    }
  }
...
int setupStreamSocket(UsageEnvironment& env,
  ...
  if (setsockopt(newSocket, SOL_SOCKET, SO_REUSEADDR,
    ...
    socketErr(env, "setsockopt(SO_REUSEADDR) error: ");
    closeSocket(newSocket);
    qDebug()<<"setsockopt SO_REUSEADDR error,closeSocket :"<<newSocket;
    return -1;
  }
  ...
  if (setsockopt(newSocket, SOL_SOCKET, SO_REUSEPORT,
    ...
    socketErr(env, "setsockopt(SO_REUSEPORT) error: ");
    closeSocket(newSocket);
    qDebug()<<"setsockopt SO_REUSEPORT error,closeSocket :"<<newSocket;
    return -1;
  }
#endif
#endif
  ...
  if (port.num() != 0 || ReceivingInterfaceAddr != INADDR_ANY) {
    ...
    if (::bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
      ...
      socketErr(env, tmpBuffer);
      closeSocket(newSocket);
      qDebug()<<"setsockopt bind error,closeSocket :"<<newSocket;
      return -1;
    }
#if defined(__WIN32__) || defined(_WIN32)
#else
  ...
  if (makeNonBlocking) {
    if (!makeSocketNonBlocking(newSocket)) {
      ...
      closeSocket(newSocket);
      qDebug()<<"failed to make non-blocking, closeSocket :"<<newSocket;
      return -1;
    }
  }
...
int readSocket(UsageEnvironment& env,
  ...
  if (bytesRead < 0) {
    ...
    //ECONNABORTED=106,WSAECONNABORTED=10053
    qDebug()<<"recvfrom"<<socket<<" error:"<<err;
    //##### END HACK
  ...
  //if(bytesRead==4)
  qDebug()<<"recvfrom:"<<socket<<" size:"<<bytesRead<<" buff:"<<QString((const char*)buffer);
  return bytesRead;
}
...
Boolean writeSocket(UsageEnvironment& env,
		    int socket, struct in_addr address, portNumBits portNum,
		    unsigned char* buffer, unsigned bufferSize) {
    qDebug()<<"writeSocket sendto:"<<socket <<" size:"<<bufferSize <<"buff:"<<QString((const char *)buffer);
  do {
...
netAddressBits ourIPAddress(UsageEnvironment& env) {
  ...
  if (ourAddress == 0) {
    ...
    if (sock >= 0) {
      ...
      closeSocket(sock);
      qDebug()<<"ourIPAddress closeSocket :"<<sock;
    }
